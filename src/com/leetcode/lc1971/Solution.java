...
Input: nnew int[][] = 3, edges = [[0,1}, {1,2}, {2,0]], source = 0, destination = 2
Output: true
...
Input: nnew int[][] = 6, edges = [[0,1}, {0,2}, {3,5}, {5,4}, {4,3]], source = 0, destination = 5
Output: false
...
static bool ValidPath(int n, int[][] edges, int source, int destination)
{
    ...
    HashSet<int> currSetX;
    HashSet<int> currSetY;
    for (int i = 0; i < edges.Length; i++)
    {
        currSetX = getHashSet(sets, edges[i][0]);
        currSetY = getHashSet(sets, edges[i][1]);
        if (currSetX == null && currSetY == null)
        {
            currSetX = new HashSet<int>();
            sets.Add(currSetX);
            currSetX.Add(edges[i][0]);
            currSetX.Add(edges[i][1]);
            if (currSetX.Contains(source) && currSetX.Contains(destination))
                return true;
        }
        else if (currSetX == null && currSetY != null)
        {
            currSetY.Add(edges[i][0]);
            if (currSetY.Contains(source) && currSetY.Contains(destination))
                return true;
        }
        else if (currSetX != null && currSetY == null)
        {
        ...
        else if (currSetX != null && currSetY != null && currSetX != currSetY)
        {
            AddHashSet(currSetX, currSetY);
            sets.Remove(currSetY);
            if (currSetX.Contains(source) && currSetX.Contains(destination))
                return true;
        }
    }
    return false;
    //currSetX = getHashSet(sets, source);
    //currSetY = getHashSet(sets, destination);
    //return getHashSet(sets, source) == getHashSet(sets, destination);
}

static void AddHashSet(HashSet<int> x, HashSet<int> y)
{
    foreach(int i in y)
    {
        x.Add(i);
...
static HashSet<int> getHashSet(List<HashSet<int>> list, int x)
{
