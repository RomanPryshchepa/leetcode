...
public class Solution {
  ...
  public int numberOfBeams(String[] bank) {
    if (bank == null || bank.length < 2)
      return 0;
    var result = 0;
    var prev = getSecurityDeviceCount(bank[0]);
    for (int i = 1; i < bank.length; i++) {
      if (prev == 0) {
        prev = getSecurityDeviceCount(bank[i]);
        continue;
      }
      var curr = getSecurityDeviceCount(bank[i]);
      if (curr == 0)
        continue;
      result += prev * curr;
      prev = curr;
  ...
  private int getSecurityDeviceCount(String row) {
    var result = 0;
