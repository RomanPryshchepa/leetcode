...
public class Solution {
    ...
    public int minimumPushes(String word) {
        ...
        for (var letter : word.toCharArray())
            letterCounters[letter - 97]++;
        Arrays.sort(letterCounters);
        var result = 0;
        var idx = letterCounters.length - 1;
        while (idx >= 0 && letterCounters[idx] > 0)
            result += (1 + (letterCounters.length - 1 - idx) / 8) * letterCounters[idx--];
        return result;
    }
}
